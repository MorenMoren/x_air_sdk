...
class CANArmReader:
    ...
    重要:
        ...
        - 调用 set_callback_mode_all() — 让新 handle 的 SDK 状态机知道要解析 STATE 回调帧
          (对已在 STATE 模式的电机是幂等的，不影响遥操作)
        - warmup 期间多次 recv_all 让 socket buffer 中的帧流入后再声明连接成功
    """
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            # 告知 SDK handle 期望接收 STATE 回调帧。
            # 电机已由遥操脚本使能并处于 STATE 模式，此调用是幂等的，
            # 不会打断遥操控制流，但对新 handle 来说是必须的，否则
            # recv_all 捕获到帧后不会更新内部位置缓存。
            self.arm.set_callback_mode_all(oa.CallbackMode.STATE)

            # Warmup: 等待 socket buffer 中的 STATE 帧流入，参考 sdk_python_basic_test.py
            self.arm.refresh_all()
            for _ in range(20):
                self.arm.recv_all(timeout_us=2000)
                time.sleep(0.05)
            ...
            if any(p != 0.0 for p in positions):
                ...
                # 全零说明 warmup 期间没收到任何帧，CAN 链路可能有问题
                self._logger.warning(
                    f"⚠️  {self.name} CAN 初始化后所有关节位置均为 0，"
                    "可能 CAN 接口不通或遥操脚本尚未启动，继续尝试运行"
                )
                self._connected = True
                return True
...
class XArmROSCollector(Node):
    ...
    def __init__(
        ...
        self._can_reader_timestamps: Dict[str, Dict[str, object]] = {}
    ...
    def _create_lerobot_dataset(self) -> LeRobotDataset:
        ...
        features.update({
            ...
            # ⭐ 绝对时间戳 (Unix epoch seconds, float64) — 每个数据源独立记录
            'timestamp.chest': {
                'dtype': 'float64',
                'shape': (1,),
                'names': ['unix_epoch_s'],
        ...
        # 手腕相机时间戳
        if self.use_wrist_camera:
            features['timestamp.wrist_left'] = {
                'dtype': 'float64',
                'shape': (1,),
                'names': ['unix_epoch_s'],
            }
            features['timestamp.wrist_right'] = {
                'dtype': 'float64',
                'shape': (1,),
                'names': ['unix_epoch_s'],
            }
    ...
    def _read_can_data(self) -> Dict[str, Optional[np.ndarray]]:
        ...
        for role, reader in self._can_readers.items():
            # ⭐ 每路 CAN 独立记录绝对时间戳 (Unix epoch seconds)
            can_abs_ts = time.time()
            can_abs = self.get_clock().now()
            results[role] = reader.read_positions()
            self._can_reader_timestamps[role] = {
                'abs_time': can_abs_ts,
                'ros_time': self.get_clock().now(),
            }
            cur_can_time = self.get_clock().now()
            self.get_logger().info(
                f"{role} can time consuming {(cur_can_time - can_abs).nanoseconds / 1e6:.1f} ms"
             )
        return results
    def _read_can_data_par(self) -> Dict[str, Optional[np.ndarray]]:
        """从所有 CAN 接口并行读取关节+夹爪数据

        使用 ThreadPoolExecutor 并行读取4路 CAN 接口，每路独立 refresh+recv，
        互不阻塞，大幅降低总延迟（总耗时 ≈ max(各路耗时) 而非 sum(各路耗时)）。
        ...
        def _read_single_can(role: str, reader: CANArmReader):
            """单个 CAN 接口读取（在线程池中执行，每路独立 CAN 总线无竞争）"""
            can_abs_ts = time.time()
            t_start = time.perf_counter()
            data = reader.read_positions()
            t_elapsed_ms = (time.perf_counter() - t_start) * 1000.0
            return role, data, can_abs_ts, t_elapsed_ms

        # 并行读取所有 CAN 接口 (4路同时 refresh+recv，而非串行等待)
        with ThreadPoolExecutor(max_workers=len(self._can_readers)) as executor:
            futures = {
                executor.submit(_read_single_can, role, reader): role
                for role, reader in self._can_readers.items()
            }
            for future in as_completed(futures):
                role, data, abs_ts, elapsed_ms = future.result()
                results[role] = data
                self._can_reader_timestamps[role] = {
                    'abs_time': abs_ts,
                    'ros_time': self.get_clock().now(),
                ...
                #self.get_logger().info(
                 #   f"{role} can time consuming {elapsed_ms:.1f} ms"
                #)
    ...
    def _read_camera_frames(self) -> Dict[str, np.ndarray]:
        ...
        时间戳存到 self.frame_timestamps (每个相机独立记录绝对时间)
        """
        frames = {}

        for cam_name, camera in self.cameras.items():
            try:
                # ⭐ 每帧数据源采样的时间戳 写入数据集 (Unix epoch seconds)
                

                # 优先使用 async_read (非阻塞), 回退到 read() (阻塞)
                result = camera.read_latest()
                abs_ts=camera.latest_timestamp
                if self.use_depth_camera:
                ...
                if self.use_depth_camera and cam_name == 'cam_chest':
                    ...
                    if cam_name == 'cam_wrist_left':
                        ...
                        # ⭐ 左腕相机绝对时间戳
                        self.frame_timestamps['timestamp.wrist_left'] = abs_ts
                    elif cam_name == 'cam_wrist_right':
                        key = 'observation.images.cam_wrist_right'
                        # ⭐ 右腕相机绝对时间戳
                        self.frame_timestamps['timestamp.wrist_right'] = abs_ts
                    else:
                        # cam_chest (无深度模式)
                        key = 'observation.images.cam_chest'
                        self.frame_timestamps['timestamp.chest'] = abs_ts
    ...
    def _collect_can_frame(self) -> bool:
        ...
        """
        can_data = self._read_can_data_par()
        ...
        # ⭐ 取 follower 读数的平均绝对时间戳作为 observation.state 的时间戳
        follower_tss = []
        for role in ['left_follower', 'right_follower']:
            ts_info = self._can_reader_timestamps.get(role, {})
            if 'abs_time' in ts_info:
                follower_tss.append(ts_info['abs_time'])
        ts_state = float(np.mean(follower_tss)) if follower_tss else time.time()

        # ⭐ 取 leader 读数的平均绝对时间戳作为 action 的时间戳
        leader_tss = []
        for role in ['left_leader', 'right_leader']:
            ts_info = self._can_reader_timestamps.get(role, {})
            if 'abs_time' in ts_info:
                leader_tss.append(ts_info['abs_time'])
        ts_action = float(np.mean(leader_tss)) if leader_tss else time.time()
        ...
        self.frame_timestamps['timestamp.state'] = ts_state
        ...
        self.frame_timestamps['timestamp.action'] = ts_action
    ...
    def _collect_frame_callback(self):
        ...
        with self.lock:
            ...
            # 构建帧数据
                ...
                # ⭐ 绝对时间戳写入数据集 (Unix epoch seconds, float64)
                'timestamp.chest': np.array([self.frame_timestamps.get('timestamp.chest', time.time())], dtype=np.float64),
                'timestamp.state': np.array([self.frame_timestamps.get('timestamp.state', time.time())], dtype=np.float64),
                'timestamp.action': np.array([self.frame_timestamps.get('timestamp.action', time.time())], dtype=np.float64),
            }
            ...
            # 手腕相机
                if self.use_wrist_camera:
                    ...
                    # ⭐ 手腕相机时间戳
                    frame['timestamp.wrist_left'] = np.array(
                        [self.frame_timestamps.get('timestamp.wrist_left', time.time())], dtype=np.float64
                    )
                    frame['timestamp.wrist_right'] = np.array(
                        [self.frame_timestamps.get('timestamp.wrist_right', time.time())], dtype=np.float64
                    )
