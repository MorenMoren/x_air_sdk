...
class XArmROSCollector(Node):
    ...
    def __init__(
        ...
        # ROS2 发布器: 独立线程通过队列异步发布，不阻塞录制回调
        self._chest_pub = None
        self._pub_queue: queue.Queue = queue.Queue(maxsize=1)  # 只保留最新帧
        self._pub_thread: Optional[threading.Thread] = None
        if enable_gui:
            self._chest_pub = self.create_publisher(
                Image, '/cam_chest/monitor/image_raw', 30
            )
            self._pub_thread = threading.Thread(
                target=self._publish_loop, daemon=True, name='chest_pub'
            )
    ...
    def cleanup(self):
        ...
        try:
            ...
            if self._chest_pub is not None:
                self.destroy_publisher(self._chest_pub)
                self._chest_pub = None
                print("🖥️  Chest monitor publisher destroyed", flush=True)
...
示例:
  ...
  python xarm_ros2_record.py --repo-id myuser/xarm_dataset \\
    ...
    parser.add_argument('--enable-gui', action='store_true', default=False,
                       help='Publish chest camera to /cam_chest/monitor/image_raw for rqt_image_view')
