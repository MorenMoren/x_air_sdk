...
# CAN接口配置（左右臂）
CAN_INTERFACES = {'left_arm': 'can3', 'right_arm': 'can4'}
...
class XArmDirectDeployer(Node):
    ...
    def __init__(
        ...
        # Action chunking 参数
        no_action_chunking: bool = False,
        n_action_steps_override: Optional[int] = None,
    ):
        ...
        # Action chunking 配置
        self.no_action_chunking = no_action_chunking
        self.n_action_steps_override = n_action_steps_override
        ...
        # Action chunking 队列 (diffusion policy 等支持多步推理的策略)
        self.action_queue = collections.deque()  # 待执行的动作队列
        self.use_action_chunking = False          # 是否启用 chunking 模式（策略加载后自动检测）
        self.n_action_steps = 1                   # 每轮推理产出的动作数
        self.refill_threshold = 0                 # 队列低于此阈值时触发新一轮推理
    ...
    def _load_policy(self):
        ...
        try:
            ...
            # 🔑 自动检测 action chunking 支持
            n_action_steps_from_cfg = getattr(config, 'n_action_steps', 1)
            if self.n_action_steps_override is not None:
                self.n_action_steps = self.n_action_steps_override
                self.get_logger().info(
                    f"Action chunking n_action_steps overridden: "
                    f"{n_action_steps_from_cfg} → {self.n_action_steps}"
                )
            else:
                self.n_action_steps = n_action_steps_from_cfg

            self.use_action_chunking = (
                not self.no_action_chunking and self.n_action_steps > 1
            )
            if self.use_action_chunking:
                self.refill_threshold = max(1, self.n_action_steps // 4)
                self.get_logger().info(
                    f"🎬 Action chunking ENABLED: "
                    f"{self.n_action_steps} steps per inference, "
                    f"refill_threshold={self.refill_threshold}"
                )
            else:
                self.get_logger().info(
                    f"Action chunking DISABLED "
                    f"(n_action_steps={self.n_action_steps}, "
                    f"no_chunking={self.no_action_chunking})"
                )
    ...
    def _inference_callback(self):
        ...
        # 🔑 Chunking 模式: 队列充足时跳过昂贵的模型前向推理
        if self.use_action_chunking:
            queue_size = len(self.action_queue)
            if queue_size > self.refill_threshold:
                return
                #pass
        ...
        try:
            ...
            if self.use_action_chunking:
                # ===== Chunking 模式: 一次推理产出 n_action_steps 个动作 =====
                n_enqueued = self._run_inference_chunked(observation)
                ...
                if True:
                    self.get_logger().info(
                        f"🎬 Chunked inference #{self.inference_count}: "
                        f"enqueued {n_enqueued} actions, "
                        f"queue={len(self.action_queue)}/{self.n_action_steps}, "
                        f"inference={inference_time*1000:.1f}ms"
                    )
            else:
                # ===== 非 Chunking 模式: 原逐帧推理逻辑 =====
                ...
                if self.inference_count % 30 == 0:
                    ...
                    self.get_logger().info(
                        ...
                        f"action: {action['action'] if 'action' in action else 'N/A'}"
                    )
    ...
    def _control_callback(self):
        ...
        Chunking 模式 (diffusion policy):
            从 action_queue 中 popleft 取动作，原样发送给硬件。
            diffusion 生成的轨迹已具备时序一致性，不做帧间插值。

        非 Chunking 模式 (ACT 等逐帧策略):
            在 current_action → target_action 之间线性插值，
            interpolation_steps 步内完成平滑过渡。

        两种模式都保留 MIT 子步骤发送 (mit_sends_per_control_cycle 次
        @ mit_interval 间隔)，确保 MIT 控制器以 ~500Hz 接收刷新指令。
        """
        if not self.is_running:
            return
        ...
        if self.use_action_chunking:
            ...
            if 'action' in action:
                ...
                if len(jp) >= 14:
                    ...
                    self.joint_data_log['left_joint1'].append(float(jp[0]))
                    self.joint_data_log['left_joint2'].append(float(jp[1]))
                    self.joint_data_log['right_joint1'].append(float(jp[7]))
                    self.joint_data_log['right_joint2'].append(float(jp[8]))
                    self.joint_data_log['control_step'].append(self.control_step_counter)

            # MIT 子步骤: 以 mit_frequency Hz 重复发送同一动作
            for send_idx in range(self.mit_sends_per_control_cycle):
                try:
                    hw_start = time.perf_counter()
                    self._send_to_hardware(action)
                    self.hardware_times.append(time.perf_counter() - hw_start)
                except Exception as e:
                    self.get_logger().error(
                        f"Control error (send {send_idx + 1}/"
                        f"{self.mit_sends_per_control_cycle}): {e}",
            ...
            # ===== 非 Chunking 模式: 插值平滑（原有逻辑，bug 已修复） =====
            with self.lock:
                ...
                for send_idx in range(self.mit_sends_per_control_cycle):
                    alpha = min(
                        ...
                        (self.interpolation_counter
                         + send_idx / self.mit_sends_per_control_cycle)
                        / self.interpolation_steps,
                    )

                    interpolated_action = self._interpolate_action(
                        self.current_action, self.target_action, alpha
                    )

                    # 记录关节数据（用于可视化）
                    if 'action' in interpolated_action:
                        jp = interpolated_action['action']
                        if len(jp) >= 14:
                            self.joint_data_log['timestamps'].append(time.time())
                            self.joint_data_log['left_joint1'].append(float(jp[0]))
                            self.joint_data_log['left_joint2'].append(float(jp[1]))
                            self.joint_data_log['right_joint1'].append(float(jp[7]))
                            self.joint_data_log['right_joint2'].append(float(jp[8]))
                            self.joint_data_log['control_step'].append(
                                self.control_step_counter
                            )
    ...
    def _run_inference_chunked(self, observation: Dict) -> int:
        """执行策略推理，产出 n_action_steps 个动作并全部入队。

        对 diffusion policy：使用 drain loop（reset → select_action × N）。
        diffusion 内部有 action deque，第 1 次 select_action 触发去噪推理，
        后续 N-1 次直接从内部队列 pop（极快）。

        对其它 n_action_steps > 1 的策略：直接调用 predict_action_chunk
        一次性获取 (B, N, D) 张量，然后逐个拆解入队。
        ...
        if self.inference_count == 0:
            self.get_logger().info("=" * 60)
            self.get_logger().info("🔍 FIRST CHUNKED INFERENCE - OBSERVATION DATA:")
            for key, value in observation.items():
                self.get_logger().info(f"  {key}: shape={value.shape}, dtype={value.dtype}")
            self.get_logger().info("=" * 60)

        # ---- 转换 observation 为 torch batch (与 _run_inference 一致) ----
        batch = {}
        for key, value in observation.items():
            if value.dtype == np.uint8:
                value = value.astype(np.float32) / 255.0
        ...
        # ---- 生成 action chunk ----
        policy_type = getattr(self.policy.config, 'type', 'unknown')

        if policy_type == 'diffusion':
            # Diffusion policy: drain loop 方式
            # reset() 清空内部 obs/action 队列，第 1 次 select_action 触发
            # 完整的去噪推理，后续 select_action 直接从内部 deque pop
            self.policy.reset()
            raw_actions = []
            with torch.no_grad():
                for _ in range(self.n_action_steps):
                    raw_actions.append(self.policy.select_action(batch))
        else:
            # 其它策略 (ACT 等): 直接调用 predict_action_chunk
            with torch.no_grad():
                chunk = self.policy.predict_action_chunk(batch)
            # chunk shape: (1, n_action_steps, action_dim)
            if chunk.ndim == 3 and chunk.shape[0] == 1:
                chunk = chunk[0]  # → (n_action_steps, action_dim)
            raw_actions = [chunk[i:i + 1] for i in range(min(chunk.shape[0], self.n_action_steps))]

        # ---- 逐个后处理并入队 ----
        prev_last_action = self.last_action
        enqueued = 0

        for step_idx, raw_output in enumerate(raw_actions):
            # 后处理（反归一化）
            action_output = self.postprocessor(raw_output)

            # 转换为 numpy action dict（与 _run_inference 相同的拆解逻辑）
            action: Dict = {}
            if isinstance(action_output, dict):
                for key, value in action_output.items():
                    if isinstance(value, torch.Tensor):
                ...
                if isinstance(action_output, torch.Tensor):
                    ...
                    if len(action_output) == 16:
                        ...
                        action['action'] = np.concatenate(
                            [action_output[:7], action_output[8:15]], axis=0
                        )
                        action['action.gripper'] = np.array(
                            [action_output[7], action_output[15]]
                        )
                    elif len(action_output) == 14:
                        action['action'] = action_output
                    else:
            ...
            # 🔑 Chunk 边界平滑: 仅对 chunk 的第一个动作与上一个 chunk
            #    的最后一个动作做 EMA 混合。chunk 内部动作保持原样，
            #    因为 diffusion 生成的轨迹已具备时序一致性。
            if (
                step_idx == 0
                and prev_last_action is not None
                and self.smoothing_alpha < 1.0
                and 'action' in action
                and 'action' in prev_last_action
            ):
                cur = np.array(action['action'], dtype=np.float32)
                prev = np.array(prev_last_action['action'], dtype=np.float32)
                action['action'] = (
                    self.smoothing_alpha * cur
        ...
        # 更新 self.last_action 为本次 chunk 的最后一个动作，
        # 供下一轮 chunk 边界平滑使用
        if self.action_queue:
            last_entry = self.action_queue[-1]
            self.last_action = {
    ...
    def _start_execution(self, go_home_first: bool = False):
        ...
        # 重置策略内部状态 (diffusion 的 obs/action 队列等)
        if hasattr(self.policy, 'reset'):
            self.policy.reset()

        self.is_running = True
        mode_str = "CHUNKING" if self.use_action_chunking else "INTERPOLATION"
        self.get_logger().info(f"🚀 Policy execution STARTED ({mode_str} mode)")
...
def main():
    ...
    # Action chunking 参数 (diffusion policy 等)
    parser.add_argument('--no-action-chunking', action='store_true', default=False,
                       help='Disable action chunking (force single-step inference even '
                            'if policy supports n_action_steps > 1)')
    parser.add_argument('--n-action-steps', type=int, default=None,
                       help='Override n_action_steps from policy config '
                            '(e.g., set to 16 to execute fewer actions per chunk)')
    ...
    deployer = XArmDirectDeployer(
        ...
        # Action chunking
        no_action_chunking=args.no_action_chunking,
